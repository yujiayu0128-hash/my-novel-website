...
function displayBooks(booksToShow) {
    ...
    if (booksToShow.length === 0) {
        bookList.innerHTML = '<div class="no-results">📖 没有找到小说</div>';
        return;
    }
    let html = '';
    booksToShow.forEach(book => {
        const name = book.name.replace('.txt', '');
        html += `
            <div class="book-item">
                <div class="book-info">
                ...
                <div class="book-actions">
                    <button class="action-btn" onclick="readBook('${book.name}')">阅读</button>
                    <button class="action-btn" onclick="downloadBook('${book.name}')">下载</button>
                </div>
            </div>
        `;
    });
...
function searchBooks() {
    ...
    if (!text) { displayBooks(books); return; }
    const filtered = books.filter(book => book.name.toLowerCase().includes(text));
